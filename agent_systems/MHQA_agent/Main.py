...
def main():
    ...
    from .Tools import calculate_trajectory_reward
    reward = calculate_trajectory_reward(traj)
    traj['reward'] = reward
    traj['success'] = reward['total'] > 0.5  # Simple success threshold
    
    save_trajectory(traj, out_path)
    print(f"[ok] saved trajectory -> {out_path}")
    print(f"[reward] total: {reward['total']:.3f}, completion: {reward['completion']:.3f}, quality: {reward['quality']:.3f}")
