...
def execute_dynamic_sampling_agent(temperature=0):
    ...
    # Track valid time range for tree sampling
    valid_start_seconds = 0
    valid_end_seconds = video_duration_seconds
    ...
    # Generate valid timestamp options based on current range
    def generate_timestamp_options(start_seconds, end_seconds):
        options = []
        for sec in range(start_seconds, end_seconds + 1):
            options.append(seconds_to_timestamp(sec))
        return options
    
    # Define JSON schemas for responses
    def get_analysis_schema():
        timestamp_options = generate_timestamp_options(valid_start_seconds, valid_end_seconds)
        return {
            "type": "object",
            "properties": {
                ...
                "decision": {
                    ...
                    "properties": {
                        ...
                        "start_timestamp": {"type": ["string", "null"], "description": "Timestamp in mm:ss format", "enum": timestamp_options},
                        "end_timestamp": {"type": ["string", "null"], "description": "Timestamp in mm:ss format", "enum": timestamp_options},
    ...
    def get_answer_more_frames_schema():
        timestamp_options = generate_timestamp_options(valid_start_seconds, valid_end_seconds)
        return {
            "type": "object",
            "properties": {
    ...
    system_prompt = (
        ...
        f"Initially, the valid range for timestamps is from 00:00 to {duration_str}. "
        f"As you go deeper down the tree of frames, the valid range for timestamps will shrink to allow you to zoom in on the relevant segments. "
        ...
        if "gemini" in model_name:
            response = client.models.generate_content(
                ...
                config=types.GenerateContentConfig(
                    ...
                    response_schema=get_answer_more_frames_schema() if allow_sample else final_answer_schema
                )
            )
            return response
        else:  # OpenAI models
            ...
            response = client.chat.completions.create(
                ...
                response_format={ "type": "json_schema", "json_schema": {"name": "answer_more_frames" if allow_sample else "final_answer",
                                                                         "strict": True, "schema": get_answer_more_frames_schema() if allow_sample else final_answer_schema}},
                max_tokens=3000,
            )
    ...
    def sample_more_frames(response_json):
        nonlocal valid_start_seconds, valid_end_seconds
        ...
        # Update valid range for tree sampling
        valid_start_seconds = start_seconds
        valid_end_seconds = end_seconds
        ...
        print(f"Valid time range updated to: {seconds_to_timestamp(valid_start_seconds)} - {seconds_to_timestamp(valid_end_seconds)}")
            ...
            if "gemini" in model_name:
                response = client.models.generate_content(
                    ...
                    config=types.GenerateContentConfig(
                        ...
                        safety_settings=gemini_safety_settings,
                        response_mime_type='application/json',
                        response_schema=get_answer_more_frames_schema() if asked_question else get_analysis_schema()
                    )
                )
            else:  # OpenAI models
                formatted_messages = format_message_for_openai(message_content)
                response = client.chat.completions.create(
                    ...
                    response_format={ "type": "json_schema", "json_schema": {"name": "analysis", "strict": True, "schema":
                                                                             get_answer_more_frames_schema() if asked_question else get_analysis_schema()}},
                    max_tokens=3000
                )
                print(f"Prompt tokens: {response.usage.prompt_tokens}")
                print(f'Cached prompt tokens: {response.usage.prompt_tokens_details.cached_tokens}')
...
def main():
    ...
    if args.dataset == "egoschema":
        os.environ["QUESTION_FILE_PATH"] = f"data/egoschema/{args.partition}_tree_sampling_{args.model}_max_iter_{args.max_iterations}.json"
        ...
        os.environ["QUESTION_FILE_PATH"] = "data/nextqa/val_tree_sampling.json"
        ...
        os.environ["QUESTION_FILE_PATH"] = "data/momaqa/test_tree_sampling.json"
