...
def depth_expansion(img_feats, image_dir,relevance_score, width_res):
    device = "cuda" if torch.cuda.is_available() else "cpu" 
    cap_score_data = relevance_score
    width_res_data = width_res
    width_cluster_id_dict = {item['name']: item['cluster_ids_x'] for item in width_res_data}

    depth_expansion_res = []

    example_path = Path(image_dir)
    ...
    depth_expansion_res.append({"name": example_path.name, "sorted_values": closest_points_temporal_subsub, "relevance": relevance_scores})

    return depth_expansion_res



if __name__ == '__main__':
    depth_expansion(img_feats, image_dir,relevance_score, width_res)
