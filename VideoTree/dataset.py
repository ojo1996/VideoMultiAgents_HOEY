...
class BaseDataset(Dataset):
    def __init__(self, args, quids_to_include=None, num_examples_to_run=-1):
        '''
        ...
        data = self.build()
        data = self.filter(data, quids_to_include, num_examples_to_run)
        self.data = data

    def set_ukey(self, name):
        self.ukey = name

    def filter(self, data, quids_to_include, num_examples_to_run):
        if quids_to_include is not None:
            data = [el for el in data if el[self.ukey] in quids_to_include]
...
class EgoSchemaDataset(BaseDataset):
    def __init__(self, args, quids_to_include=None, num_examples_to_run=-1):
        self.set_ukey('uid')
        super().__init__(args, quids_to_include=quids_to_include, num_examples_to_run=num_examples_to_run)
...
def get_dataset(args, quids_to_include=None, num_examples_to_run=-1):
    if args.dataset == 'egoschema':
        return EgoSchemaDataset(args, quids_to_include=quids_to_include, num_examples_to_run=num_examples_to_run)
    else:
        return NextDataset(args, quids_to_include=quids_to_include, num_examples_to_run=num_examples_to_run)
